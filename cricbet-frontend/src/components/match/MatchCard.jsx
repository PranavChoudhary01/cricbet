  ...
  return (
    <div className="card animate-fade-up" style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <span style={{ fontSize: 11, color: 'var(--text-secondary)', background: 'var(--bg-elevated)', padding: '3px 10px', borderRadius: 20 }}>
          {match.match_type} {match.venue ? `· ${match.venue}` : ''}
        </span>
        {isLive ? (
          <span className="badge badge-live">
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', animation: 'pulse-dot 1.5s ease-in-out infinite', display: 'inline-block' }} />
            LIVE
          </span>
        ) : (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
        <div style={{ flex: 1 }}>
          ...
          {match.score_a && (
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--accent)' }}>{match.score_a}</div>
          )}
        </div>

        <div style={{ width: 36, height: 36, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 700, color: 'var(--text-muted)' }}>
          VS
        </div>
        ...
        <div style={{ flex: 1, textAlign: 'right' }}>
          ...
          {match.score_b && (
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--accent)' }}>{match.score_b}</div>
          )}
        </div>
      </div>
        ...
        {['team_a_win', 'draw', 'team_b_win'].map((type) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 17, fontWeight: 600, color: selected ? 'var(--accent)' : flashing ? 'var(--accent)' : 'var(--text-primary)', transition: 'color 0.2s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10 }}>
        <button
          style={{ padding: '7px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer' }}
        ...
        <button
          onClick={() => navigate(`/trading/${match.id}`, { state: { match } })}
          style={{ padding: '7px', background: 'var(--accent-glow)', border: '1px solid var(--border-accent)', borderRadius: 8, fontSize: 12, color: 'var(--accent)', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer' }}
        >
          📈 Trade
        </button>
      </div>
